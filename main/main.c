...
static const char *tag = "MAX30102_NimBLE_BLE_PULSE_OXIMETER";
...
static const char *device_name = "bleplx_max30102_1.0";

static int blehr_gap_event(struct ble_gap_event *event, void *arg);

static uint8_t blehr_addr_type;

/* Variable to read heart beats */
// static uint8_t heartrate = 90;
static uint16_t oxygensaturation = 90;
static uint16_t heartbpm  = 77;
...
/**
 * Addition by @author Joshua D JOHN
 * Function to initialize the I2C port connecting the MAX30102
*/
    ...
    */
    ...
    while(notify_state) {
        ...
        if(result.pulse_detected) {
            ...
            oxygensaturation = (uint16_t)result.spO2;
            heartbpm = (uint16_t)result.heart_bpm;
            memcpy(&plxm[index], &oxygensaturation, sizeof(oxygensaturation));
            index += sizeof(oxygensaturation);
            memcpy(&plxm[index], &heartbpm, sizeof(heartbpm));
            om = ble_hs_mbuf_from_flat(plxm, sizeof(plxm));
            rc = ble_gattc_notify_custom(conn_handle, plx_contm_handle, om);
        }
        //Update rate: 100Hz
...
blehr_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
        MODLOG_DFLT(INFO, "subscribe event; cur_notify=%d\n value handle; "
                    "val_handle=%d\n",
                    event->subscribe.cur_notify, plx_contm_handle);
        if (event->subscribe.attr_handle == plx_contm_handle) {
            notify_state = event->subscribe.cur_notify;
            blehr_tx_hrate_reset();
        } else if (event->subscribe.attr_handle != plx_contm_handle) {
            notify_state = event->subscribe.cur_notify;
            blehr_tx_hrate_stop();
        }
...
void app_main(void)
{
    ...
    /** 
     * Adding an actual Sensor, MAX30102
